...
class Index extends BasicApi
{
    ...
    public function install()
    {
        ...
        if (is_file($lockFile)) {
            $err = "当前已经安装{$this->siteName}，如果需要重新安装，请手动移除install/install.lock文件";
            ...
            if (!$err) {
                $err = '当前权限不足，无法写入配置文件config/database.php';
            }
        }
        if ($err) {
            $this->error($err);
        ...
        try {
            ...
            set_time_limit(0);
            //检测能否读取安装文件
            $sql = @file_get_contents($dataPath . 'install.sql');
            if (!$sql) {
            ...
            if (!$result) {
                throw new Exception("无法写入安装锁定到install/install.lock文件，请检查是否有写权限");
            }
            if ($initData) {
                $this->initData();
            }
